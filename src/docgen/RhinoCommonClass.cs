...
namespace docgen
{
    ...
    class RhinoCommonClass
    {
        ...
        static string BindingTypeToRhinoCommonType(string bindingType)
        {
            if (bindingType.IndexOf("BND_Circle") >= 0)
                return "Circle";
            return bindingType;
        }
        ...
        public Tuple<ConstructorDeclarationSyntax, DocumentationCommentTriviaSyntax> GetConstructor(string[] parameterTypes)
        {
            for (int i = 0; i < Constructors.Count; i++)
            {
                ...
                if (c.ParameterList.Parameters.Count == parameterTypes.Length)
                {
                    ...
                    {
                        string ptypeAsRhinoCommon = BindingTypeToRhinoCommonType(parameterTypes[j]);
                        var ctype = c.ParameterList.Parameters[j].Type.ToString();
                        if (!ptypeAsRhinoCommon.Equals(ctype))
                        {
                            match = false;
                            break;
                        }
