...
security create-keychain -p $MACOS_KEYCHAIN_TEMP_PWD signing_temp.keychain
security default-keychain -s signing_temp.keychain
security unlock-keychain -p $MACOS_KEYCHAIN_TEMP_PWD signing_temp.keychain
security import certificate.p12 -k signing_temp.keychain -P $MACOS_CERTIFICATE_PWD -T /usr/bin/codesign
security set-key-partition-list -S apple-tool:,apple:,codesign: -s -k $MACOS_KEYCHAIN_TEMP_PWD signing_temp.keychain > /dev/null
